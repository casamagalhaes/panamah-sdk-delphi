unit PanamahSDKTests.Models.EventoCaixa;
...
type
  ...
  TTestEventoCaixaTestCase = class(TPanamahTestCase)
  private
...
{ TTestEventoCaixaTestCase }

function TTestEventoCaixaTestCase.GetFixturePath(const AFilename: string): string;
begin
  Result := Concat(GetCurrentDir, '\Models\Fixtures\', AFilename);
end;

procedure TTestEventoCaixaTestCase.OnError(Error: Exception);
begin
  Assert(False, Error.Message);
end;

procedure TTestEventoCaixaTestCase.TestSendingEventoCaixa;
var
  JSON: string;
...
initialization
  RegisterTest(TTestEventoCaixaTestCase.Suite);
