unit PanamahSDKTests.Models.Acesso;
...
type
  ...
  TTestAcessoTestCase = class(TPanamahTestCase)
  private
    function GetFixturePath(const AFilename: string): string;
    procedure OnError(Error: Exception);
  published
    procedure TestSendingAcesso;
    procedure TestSendingAcessoMultitenancy;
  end;
...
function TTestAcessoTestCase.GetFixturePath(const AFilename: string): string;
begin
  Result := Concat(GetCurrentDir, '\Models\Fixtures\', AFilename);
end;

procedure TTestAcessoTestCase.OnError(Error: Exception);
begin
  Assert(False, Error.Message);
end;

procedure TTestAcessoTestCase.TestSendingAcesso;
var
  JSON: string;
  Acesso: IPanamahAcesso;
begin
  JSON := TFile.ReadAllText(GetFixturePath('acesso.json'));
  Acesso := TPanamahAcesso.FromJSON(JSON);
...
procedure TTestAcessoTestCase.TestSendingAcessoMultitenancy;
var
  JSON: string;
  Acesso: IPanamahAcesso;
begin
  ...
  begin
    ...
    Save(Acesso, '03992843467');
    Save(Acesso, '02541926375');
    Save(Acesso, '00934509022');
    Flush;
  end;
end;

initialization
  RegisterTest(TTestAcessoTestCase.Suite);
