unit PanamahSDKTests.Models.Meta;
...
type
  ...
  TTestMetaTestCase = class(TPanamahTestCase)
  private
...
function TTestMetaTestCase.GetFixturePath(const AFilename: string): string;
begin
  Result := Concat(GetCurrentDir, '\Models\Fixtures\', AFilename);
end;

procedure TTestMetaTestCase.OnError(Error: Exception);
begin
  Assert(False, Error.Message);
end;

procedure TTestMetaTestCase.TestSendingMeta;
var
  JSON: string;
  Meta: IPanamahMeta;
begin
...
initialization
  RegisterTest(TTestMetaTestCase.Suite);
